...
export class ServiceAluno {
  ...
  private limparCamposAntigos(dados: Partial<Aluno> & any) {
    /*
      O aluno agora é vinculado ao responsável por id_responsavel
      e à escola por id_escola.
    */
  ...
  */
  ...
  async atualizar(id: number, dados: Partial<Aluno> & any, usuarioLogado?: UsuarioLogado) {
    ...
    */
    ...
    if (
      dados.id_responsavel !== undefined
    ) {
      const responsavel =
        await this.validarResponsavel(
    ...
    */
    ...
    if (
      dados.id_escola !== undefined
    ) {
      const escola =
        await this.validarEscola(
          dados.id_escola
        );
